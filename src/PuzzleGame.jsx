...
function isSolvable(board, size) {
  ...
  console.log(`[PuzzleGame] Solvability check: inversions=${inversions}, rowFromBottom=${rowFromBottom}`);
  
  // For 3x3 puzzles (odd grid size):
  // The puzzle is solvable if the number of inversions is even
  if (size % 2 === 1) {
    const isSolv = inversions % 2 === 0;
    console.log(`[PuzzleGame] Odd grid (${size}x${size}): Solvable = ${isSolv}`);
    return isSolv;
  }
  ...
  const isSolv = (rowFromBottom % 2 === 0) ? (inversions % 2 === 1) : (inversions % 2 === 0);
...
function shuffle(arr) {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    if (isSolvable(a, 3)) {
      ...
      if (solvablePuzzles % 5 === 0) { // Only check every 5th puzzle to avoid excessive computation
        const path = solvePuzzle(a, 3);
        const pathLength = path.length;
        
        console.log(`[PuzzleGame] Generated solvable puzzle #${solvablePuzzles} with difficulty: ${pathLength} moves`);
        
        // Keep track of the puzzle closest to our target difficulty
        if (Math.abs(pathLength - targetDifficulty) < Math.abs(bestPathLength - targetDifficulty)) {
          bestPuzzle = a;
      ...
      if (bestPathLength <= targetDifficulty * 1.5 && solvablePuzzles >= 10) {
        console.log(`[PuzzleGame] Found good puzzle with difficulty: ${bestPathLength} moves`);
        return bestPuzzle;
      }
    }
  }
  ...
  if (bestPuzzle) {
    console.log(`[PuzzleGame] Using best found puzzle with difficulty: ${bestPathLength} moves`);
    return bestPuzzle;
  }
...
function solvePuzzle(startBoard, size) {
  ...
  const openSet = [{ board: startBoard.slice(), cost: 0, heuristic: manhattanDistance(startBoard) }];
  ...
  while (openSet.length > 0 && explored < MAX_EXPLORED) {
    explored++;
    if (explored % 1000 === 0) {
      console.log(`[PuzzleGame] Explored ${explored} states, open set size: ${openSet.length}`);
    }
    ...
    if (isSolved(board)) {
      solution = board;
      console.log("[PuzzleGame] Found solution after exploring", explored, "states");
      break;
    }
    ...
    for (const move of moves) {
      ...
      if (!gScore.has(newBoardKey) || tentativeGScore < gScore.get(newBoardKey)) {
...
function PuzzleGame({ updateStats }) {
  ...
  useEffect(() => {
    ...
    const tilesChanged = !tiles.every((tile, idx) => tile === previousTilesRef.current[idx]);
    ...
    if (tilesChanged) {
      ...
      const isFromHint = solutionRef.current.length > 0 && solutionPath.length > 0;
      
      if (!isFromHint || isSolved(tiles)) {
        console.log("[PuzzleGame] Tiles changed independently, resetting solution path");
        solutionRef.current = [];
        setSolutionPath([]);
      }
    }
  ...
  function handleHint() {
    ...
    if (!isSolvable(tiles, size)) {
      console.log("[PuzzleGame] Current puzzle state is not solvable! Regenerating a solvable state.");
      ...
      solutionRef.current = [];
      setSolutionPath([]);
      alert("This puzzle was in an unsolvable state. A new, solvable puzzle has been generated.");
      return;
    }
    ...
    if (!solutionRef.current.length) {
      ...
      if (!path.length) {
        console.log("[PuzzleGame] No solution path found for a supposedly solvable puzzle. This shouldn't happen!");
    ...
    if (solutionRef.current.length) {
      ...
      const empty = tiles.indexOf(null);
      console.log("[PuzzleGame] Making move: Empty at", empty, "swapping with", nextMove);
      ...
      if (!isSolved(tiles)) {
        ...
        if (path.length > 0) {
          ...
          console.log("[PuzzleGame] No solution path found. Puzzle may be in an invalid state.");
        }
      }
    }
  }
